...
import { collection, query, deleteDoc, doc, getDoc, updateDoc, getDocs, where, orderBy } from "firebase/firestore";
...
function TratamientosEspecif(props) {
  ...
  const [noHayTratamientos, setNoHayTratamientos] = useState(false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const tratamientosSnapshot = await getDocs(query(tratamientosCollection.current, where("idPaciente", "==", props.id)));

        const tratamientosDataArray = tratamientosSnapshot.docs
          .map((doc) => ({
            ...doc.data(),
            id: doc.id,
          }));

        if (tratamientosDataArray.length === 0) {
          setNoHayTratamientos(true)
        }
        tratamientosDataArray.sort((a, b) => b.codigo - a.codigo);
  ...
  }, [fetchDataGral, fetchDataAndSetOptions, props.id]);
